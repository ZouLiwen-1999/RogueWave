...
def parse_args(args):
    ...
    data_dir = '../data/eu/Annotations'#存放注释文件的目录
    ...
    args_snapshot_path = '../data/eu/snapshots'
    # args_tensorboard_dir = os.path.join(root_dir(), 'logs')
    args_tensorboard_dir = '../data/eu/logs'

    csv_parser = subparsers.add_parser('csv')
    csv_parser.add_argument('--annotations', help='Path to CSV file containing annotations for training.',
                            default=args_annotations)
    csv_parser.add_argument('--classes', help='Path to a CSV file containing class label mapping.',
                            default='../data/eu/Annotations/class_mappings.csv')
    ...
    csv_parser.add_argument('--base_dir',
                            ...
                            default='../data/eu/images')#存放图片的目录
    ...
    group.add_argument('--weights', help='Initialize the model with weights from a file.',default='../data/eu/pre/resnet101_weights_tf_dim_ordering_tf_kernels.h5')
...
def main(args=None):
    ...
    # start training
    h=training_model.fit_generator(
    ...
    for value in h.history.values():
        for i in value:
          i.item()

    with open('./history.txt', 'w+') as j:
        j.write(str(h.history))
